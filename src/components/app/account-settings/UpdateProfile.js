...
class UpdateProfile extends ValidationComponent {
  ...
  async componentDidMount() {
    ...
    state.nameEn = this.props.userData.nameEn;
    state.nameAr = this.props.userData.nameAr;
    state.email = this.props.userData.email;
    state.gender = this.props.userData.gender;
    /* if (this.props.userData.profileImage) {
      state.userImage = `${BASE_URL}/users/${this.props.userData._id}/image/${
    ...
    console.log("json data", userData.id);
  }

  componentWillReceiveProps() {
    /* nextProps if (
      this.props.userData.profileImage != nextProps.userData.profileImage &&
      typeof nextProps.userData.profileImage != "undefined"
    ) {
      const state = this.state;
      state.userImage = `${BASE_URL}/users/${this.props.userData._id}/image/${
        nextProps.userData.profileImage
      }`;
      this.setState(state);
    } */
  }
      ...
      if (res.status === 204) {
        this.props.getUserProfile(requestData, response => {
          if (response.status === 200) {
            ...
            Toast.showToast(
              MESSAGES.USER_PROFILE_IMAGE_UPLOAD_SUCCESS,
              "success"
            );
          } else {
  ...
  onPressSaveProfileDetails = () => {
    ...
    if (this.isFormValid()) {
      this.props.updateUserProfileAPI(
        requestData,
  ...
  render() {
    return (
      <Container style={styles.container}>
        ...
        <HeaderComponent title="Edit Profile" leftButton="menu" />
        <Content style={styles.bgWhite}>
          {/* <View style={[styles.row, styles.bgWhite, paddingTop.Ten]}>
            <View
              style={[
                styles.verticalCenter,
                paddingBottom.Fifteen,
                paddingTop.Twenty
              ]}
            >
              <View style={styles.pRelative}>
                ...
                <TouchableOpacity
                  style={innerStyle.cameraimagewrap}
                  onPress={this.selectUserProfileImage}
                >
              ...
              <View>
                <Text style={styles.smallHeading}>Change Image</Text>
              </View>
              <View>
                <Text style={[styles.smallHeading, { paddingTop: 0 }]}>
                  {" "}
                  Max Size: 5MB
                </Text>
              </View>
          ...
          <View style={styles.formBox}>
            ...
            >
              <CommonButton
                label="SAVE"
                dynamicBtnStyle={innerStyle.btnWidth}
                onPress={this.onPressSaveProfileDetails}
              />
            </View>
